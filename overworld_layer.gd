...
var return_button_rect: Rect2 = Rect2()
...
func _input(event: InputEvent) -> void:
	if not visible or overworld_manager == null:
		return
...
func _handle_action_click(click_pos: Vector2) -> bool:
	...
	if return_button_rect.has_point(click_pos):
...
func _rebuild_layout() -> void:
	...
	var right_panel_w: float = clamp(viewport_size.x * 0.24, 300.0, 360.0)
	...
	side_panel_rect = Rect2(Vector2(viewport_size.x - right_panel_w - 20.0, top_margin), Vector2(right_panel_w, usable_h))
	var main_w: float = side_panel_rect.position.x - left_reserved - gap
	main_w = max(main_w, 280.0)
	main_panel_rect = Rect2(Vector2(left_reserved, top_margin), Vector2(main_w, usable_h))
	...
	var minimap_h: float = min(side_panel_rect.size.y * 0.52, side_panel_rect.size.y - 240.0)
	minimap_rect = Rect2(side_panel_rect.position + Vector2(minimap_margin, 34.0), Vector2(side_panel_rect.size.x - minimap_margin * 2.0, minimap_h))
	buttons_panel_rect = Rect2(Vector2(side_panel_rect.position.x + 14.0, minimap_rect.position.y + minimap_rect.size.y + 14.0), Vector2(side_panel_rect.size.x - 28.0, side_panel_rect.position.y + side_panel_rect.size.y - (minimap_rect.position.y + minimap_rect.size.y + 14.0) - 14.0))
...
func _rebuild_action_buttons() -> void:
	...
	var util_h: float = 50.0
	var y: float = buttons_panel_rect.position.y + 82.0
	...
	y += sense_h + 18.0
	return_button_rect = Rect2(Vector2(inner_x, y), Vector2(inner_w, util_h))

func _draw_main_area(font) -> void:
	if font:
		draw_string(font, main_panel_rect.position + Vector2(18, 22), "Exploration", HORIZONTAL_ALIGNMENT_LEFT, -1, 18, TEXT_COLOR)
		draw_string(font, main_panel_rect.position + Vector2(18, 44), _phase_prompt(), HORIZONTAL_ALIGNMENT_LEFT, -1, 15, MUTED_TEXT)
	...
	for dir in DIR_ORDER:
		...
		if typeof(rect_value) != TYPE_RECT2:
			continue
		var rect: Rect2 = rect_value
		draw_line(player_center, rect.get_center(), Color(1, 1, 1, 0.05), 2.0)
...
func _draw_player_core(font) -> void:
	...
	if font:
		draw_string(font, player_center + Vector2(-70, 62), "Охотник", HORIZONTAL_ALIGNMENT_CENTER, 140, 16, TEXT_COLOR)
		draw_string(font, player_center + Vector2(-100, 84), "Позиция: (%d, %d)" % [Session.player_pos.x, Session.player_pos.y], HORIZONTAL_ALIGNMENT_CENTER, 200, 14, MUTED_TEXT)
...
func _draw_direction_card(font, dir: Vector2i, rect: Rect2) -> void:
	...
	for line in title_lines:
		draw_string(font, Vector2(rect.position.x + 10.0, title_y), line, HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - 20.0, 14, TEXT_COLOR)
		title_y += 15.0
	...
	if entries.size() > max_entries and lines_left > 0:
		draw_string(font, Vector2(rect.position.x + 10.0, body_y), "+%d ещё" % (entries.size() - max_entries), HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - 20.0, 12, MUTED_TEXT)

func _draw_side_area(font) -> void:
	if font:
		draw_string(font, side_panel_rect.position + Vector2(14, 22), "Minimap", HORIZONTAL_ALIGNMENT_LEFT, -1, 18, TEXT_COLOR)
	_draw_minimap()
...
func _draw_buttons_panel(font) -> void:
	...
	draw_string(font, buttons_panel_rect.position + Vector2(10, 18), "Run status", HORIZONTAL_ALIGNMENT_LEFT, -1, 16, TEXT_COLOR)
	draw_string(font, buttons_panel_rect.position + Vector2(10, 40), Session.get_contract_progress_text(), HORIZONTAL_ALIGNMENT_LEFT, buttons_panel_rect.size.x - 20.0, 14, MUTED_TEXT)
	draw_string(font, buttons_panel_rect.position + Vector2(10, 60), "Рюкзак: %d / %d · Добыча: %d зол." % [Session.get_backpack_used(), int(Session.backpack_capacity), Session.get_carried_loot_value()], HORIZONTAL_ALIGNMENT_LEFT, buttons_panel_rect.size.x - 20.0, 14, MUTED_TEXT)

	_draw_action_button(font, sense_button_rects.get("hearing", Rect2()), "Слух", "Использовать слух", _can_use_sense(), _sense_color("hearing", 0.45))
	_draw_action_button(font, sense_button_rects.get("smell", Rect2()), "Нюх", "Использовать нюх", _can_use_sense(), _sense_color("smell", 0.45))
	_draw_action_button(font, sense_button_rects.get("echo", Rect2()), "Эхо", "Использовать эхо", _can_use_sense(), _sense_color("echo", 0.45))
	_draw_action_button(font, return_button_rect, "Вернуться в деревню", "R · завершить вылазку сейчас", Session.run_active, Color(0.95, 0.65, 0.2, 0.40))
...
func _draw_action_button(font, rect_value: Variant, title: String, subtitle: String, enabled: bool, accent: Color) -> void:
	...
	draw_string(font, rect.position + Vector2(10, 18), title, HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - 20.0, 15, title_color)
	draw_string(font, rect.position + Vector2(10, 38), subtitle, HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - 20.0, 13, subtitle_color)
...
func _draw_minimap() -> void:
	...
	for key in Session.world.keys():
		...
		var cell_rect := Rect2(board_origin + Vector2(float(local.x) * cell_px, float(local.y) * cell_px), Vector2(cell_px - 1.0, cell_px - 1.0))
		var fill_color := Color(0.18, 0.18, 0.22, 1.0)
		var border_color := Color(1, 1, 1, 0.06)

		var cell_data = Session.world[pos]
		if cell_data is Dictionary and bool(cell_data.get("visited", false)):
			fill_color = Color(0.10, 0.22, 0.16, 1.0)
			border_color = Color(0.55, 0.95, 0.70, 0.18)
	...
	var player_rect := Rect2(board_origin + Vector2(float(player_local.x) * cell_px, float(player_local.y) * cell_px), Vector2(cell_px - 1.0, cell_px - 1.0))
...
func _draw_minimap_overlay_cell(board_origin: Vector2, bounds: Rect2i, cell_px: float, pos: Vector2i, sense_type: String, is_current: bool) -> void:
	var local: Vector2i = pos - bounds.position
	var rect := Rect2(board_origin + Vector2(float(local.x) * cell_px, float(local.y) * cell_px), Vector2(cell_px - 1.0, cell_px - 1.0))
...
func _collect_entries_for_pos(into: Array, seen: Dictionary, source: Array, target_pos: Vector2i) -> void:
	for entry_value in source:
		...
		if source_pos != target_pos:
			continue
		var signature := "%s|%s|%s|%s" % [str(entry.get("sense_type", "")), str(entry.get("kind", "")), str(source_pos), str(entry.get("text", ""))]
...
func _draw_wrapped_lines(font, lines: Array[String], rect: Rect2, start_y: float, font_size: int, color: Color, max_lines: int) -> int:
	...
	for i in range(min(lines.size(), max_lines)):
		draw_string(font, Vector2(rect.position.x + 10.0, y), lines[i], HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - 20.0, font_size, color)
...
func _dir_layout_offset(dir: Vector2i) -> Vector2:
	match dir:
		Vector2i(0, -1): return Vector2(0.0, -1.10)
		Vector2i(1, -1): return Vector2(0.82, -0.78)
		Vector2i(1, 0): return Vector2(1.08, 0.0)
		Vector2i(1, 1): return Vector2(0.82, 0.78)
		Vector2i(0, 1): return Vector2(0.0, 1.10)
		Vector2i(-1, 1): return Vector2(-0.82, 0.78)
		Vector2i(-1, 0): return Vector2(-1.08, 0.0)
...
func _dir_name(dir: Vector2i) -> String:
	match dir:
		Vector2i(0, -1): return "Север"
		Vector2i(1, -1): return "Северо-восток"
		Vector2i(1, 0): return "Восток"
		Vector2i(1, 1): return "Юго-восток"
		Vector2i(0, 1): return "Юг"
		Vector2i(-1, 1): return "Юго-запад"
		Vector2i(-1, 0): return "Запад"
		Vector2i(-1, -1): return "Северо-запад"
		_: return "Направление"
...
func _sense_color(sense_type: String, alpha: float) -> Color:
	match sense_type:
		"hearing": return Color(0.2, 0.6, 1.0, alpha)
		"smell": return Color(0.6, 1.0, 0.2, alpha)
		"echo": return Color(1.0, 0.8, 0.2, alpha)
		_: return Color(1.0, 1.0, 1.0, alpha)
